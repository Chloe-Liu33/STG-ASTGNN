...
def attention(query, key, value, mask=None, dropout=None):
    ...
    if mask is not None:
        # I try to change the masked value -1e9 and I found -1e4 is OK.
...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, src, trg):
        ...
        trg: (batch, N, T_out, F_out=1)
        '''
        encoder_output = self.encode(src)  # (batch_size, N, T_in, d_model)
        # print('trg:',trg.shape)   ## batch,N,12,1
    ...
    def decode(self, trg, encoder_output):
        # f = self.decoder(self.trg_embed(trg))
        # print('encoder_output:',encoder_output.shape)   ##bxNxTxd
        # return self.prediction_generator(self.decoder(self.trg_embed(trg), encoder_output))
        return  encoder_output, self.prediction_generator(self.decoder(self.trg_embed(trg), encoder_output))
