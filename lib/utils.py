...
from .metrics import masked_mape_np,masked_mape,masked_mape2
...
## for PEMSD3-Stream adj-matrix
        ...
        adj_mx = adj_mx[ids,ids]

        adj_mx = np.pad(adj_mx,((0,num_of_vertices-adj_mx.shape[0]),(0,num_of_vertices-adj_mx.shape[1])),'constant')
        print('shape of new adj_mx',adj_mx.shape)
...
def get_adjacency_matrix_2direction(distance_df_filename, num_of_vertices, id_filename=None):
    ...
    if 'npz' in distance_df_filename:
        ...
        adj_mx = adj_mx[:, ids]
        print('adj', adj_mx.shape)
        adj_mx = np.pad(adj_mx, ((0, num_of_vertices - adj_mx.shape[0]), (0, num_of_vertices - adj_mx.shape[1])),
                        'constant')
        print('shape of new adj_mx', adj_mx.shape)
...
#
def compute_val_loss(net, val_loader, weight_sample, weight_node, criterion_train, sw, epoch):
    '''
    ...
    with torch.no_grad():
        ...
        for batch_index, batch_data in enumerate(val_loader):
            ...
            # print('decoder_input_list',decoder_input_list.shape)
            # print('decoder_start_inputs', decoder_start_inputs.shape)
            ## expected tensor as element 1 in arguement 0, but got tuple
            # decoder_input_list = torch.tensor(decoder_input_list)
            ...
            # loss = huber(labels, predict_output, 0.4)
...
def predict_and_save_results(net, data_loader, data_target_tensor, epoch, _max, _min, params_path, type):
    ...
    with torch.no_grad():
        ...
        Throughtput = length/curr_time
        print('final throughput:', Throughtput)
...
def load_graphdata_normY_channel1(graph_signal_matrix_filename,graph_signal_matrix_filename_test1,graph_signal_matrix_filename_test2,graph_signal_matrix_filename_test3, num_of_hours, num_of_days, num_of_weeks, DEVICE, batch_size, shuffle=True, percent=1.0):
    '''
    ...
    file_test0 = os.path.basename(graph_signal_matrix_filename).split('.')[0]

    dirpath_test0 = os.path.dirname(graph_signal_matrix_filename)

    filename_test0 = os.path.join(dirpath_test0,
                                  file_test0 + '_r' + str(num_of_hours) + '_d' + str(num_of_days) + '_w' + str(
                                      num_of_weeks) + '.npz')

    file_test1 = os.path.basename(graph_signal_matrix_filename_test1).split('.')[0]

    dirpath_test1 = os.path.dirname(graph_signal_matrix_filename_test1)

    filename_test1 = os.path.join(dirpath_test1,
                            file_test1 + '_r' + str(num_of_hours) + '_d' + str(num_of_days) + '_w' + str(num_of_weeks) + '.npz')
    file_test2 = os.path.basename(graph_signal_matrix_filename_test2).split('.')[0]

    dirpath_test2 = os.path.dirname(graph_signal_matrix_filename_test2)

    filename_test2 = os.path.join(dirpath_test2,
                                 file_test2 + '_r' + str(num_of_hours) + '_d' + str(num_of_days) + '_w' + str(
                                     num_of_weeks) + '.npz')
    file_test3 = os.path.basename(graph_signal_matrix_filename_test3).split('.')[0]

    dirpath_test3 = os.path.dirname(graph_signal_matrix_filename_test3)

    filename_test3 = os.path.join(dirpath_test3,
                                 file_test3 + '_r' + str(num_of_hours) + '_d' + str(num_of_days) + '_w' + str(
                                     num_of_weeks) + '.npz')

    print('load file:', filename)

    file_data = np.load(filename)
    file_data_test0 = np.load(filename_test0)
    file_data_test1 = np.load(filename_test1)
    file_data_test2 = np.load(filename_test2)
    file_data_test3 = np.load(filename_test3)

    ##dataset 0

    train_x = file_data['train_x']  # (10181, 307, 3, 12)
    train_x = train_x[:,:,0:1,:]
    # train_x = train_x[0:4032, :, 0:1, :]
    train_target = file_data['train_target']  # (10181, 307, 12)
    # train_target = train_target[0:4032,:,:]
    ...
    # ## dataset 1
    # train_x1 = file_data_test1['train_x']  # (10181, 307, 3, 12)
    # train_x1 = train_x1[:, :, 0:1, :]
    # train_target1 = file_data_test1['train_target']  # (10181, 307, 12)
    # train_timestamp1 = file_data_test1['train_timestamp']  # (10181, 1)
    #
    # train_x_length1 = train_x1.shape[0]
    # scale1 = int(train_x_length1 * percent)
    # print('ori length:', train_x_length1, ', percent:', percent, ', scale:', scale1)
    # train_x1 = train_x1[:scale1]
    # train_target1 = train_target1[:scale1]
    # train_timestamp1 = train_timestamp1[:scale1]

    ##dataset 2
    # train_x2 = file_data_test2['train_x']  # (10181, 307, 3, 12)
    # train_x2 = train_x2[:, :, 0:1, :]
    # train_target2 = file_data_test2['train_target']  # (10181, 307, 12)
    # train_timestamp2 = file_data_test2['train_timestamp']  # (10181, 1)
    #
    # train_x_length2 = train_x2.shape[0]
    # scale2 = int(train_x_length2 * percent)
    # print('ori length:', train_x_length2, ', percent:', percent, ', scale:', scale2)
    # train_x2 = train_x2[:scale2]
    # train_target2 = train_target2[:scale2]
    # train_timestamp2 = train_timestamp2[:scale2]
    # ##dataset 3
    # train_x3 = file_data_test3['train_x']  # (10181, 307, 3, 12)
    # train_x3 = train_x3[:, :, 0:1, :]
    # train_target3 = file_data_test3['train_target']  # (10181, 307, 12)
    # train_timestamp3 = file_data_test3['train_timestamp']  # (10181, 1)
    #
    # train_x_length3 = train_x3.shape[0]
    # scale3 = int(train_x_length3 * percent)
    # print('ori length:', train_x_length3, ', percent:', percent, ', scale:', scale3)
    # train_x3 = train_x3[:scale3]
    # train_target3 = train_target3[:scale3]
    # train_timestamp3 = train_timestamp3[:scale3]
    ...
    # dataset 0
    test_x0 = file_data_test0['test_x']
    test_x0 = test_x0[:, :, 0:1, :]
    test_target0 = file_data_test0['test_target']
    test_timestamp0 = file_data_test0['test_timestamp']

    # dataset 1
    test_x1 = file_data_test1['test_x']
    test_x1 = test_x1[:, :, 0:1, :]
    test_target1 = file_data_test1['test_target']
    test_timestamp1 = file_data_test1['test_timestamp']
    #dataset 2
    test_x2 = file_data_test2['test_x']
    test_x2 = test_x2[:, :, 0:1, :]
    test_target2 = file_data_test2['test_target']
    test_timestamp2 = file_data_test2['test_timestamp']
    #dataset 3
    test_x3 = file_data_test3['test_x']
    test_x3 = test_x3[:, :, 0:1, :]
    test_target3 = file_data_test3['test_target']
    test_timestamp3 = file_data_test3['test_timestamp']

    ##dataset 0
    _max = file_data['mean']  # (1, 1, 3, 1)
    _min = file_data['std']  # (1, 1, 3, 1)
    #dataset 1
    _max1 = file_data_test1['mean']  # (1, 1, 3, 1)
    _min1 = file_data_test1['std']  # (1, 1, 3, 1)

    ##dataset 2
    _max2 = file_data_test2['mean']  # (1, 1, 3, 1)
    _min2 = file_data_test2['std']  # (1, 1, 3, 1)
    ##dataset 3
    _max3 = file_data_test3['mean']  # (1, 1, 3, 1)
    _min3 = file_data_test3['std']  # (1, 1, 3, 1)
    ...
    test_target_norm0 = max_min_normalization(test_target0, _max[:, :, 0, :], _min[:, :, 0, :])

    test_target_norm1 = max_min_normalization(test_target1, _max1[:, :, 0, :], _min1[:, :, 0, :])

    test_target_norm2 = max_min_normalization(test_target2, _max2[:, :, 0, :], _min2[:, :, 0, :])

    test_target_norm3 = max_min_normalization(test_target3, _max3[:, :, 0, :], _min3[:, :, 0, :])
    ...
    #  ------- test_loader -------
    ##dataset0
    test_decoder_input_start0 = test_x0[:, :, 0:1, -1:]  # (B, N, 1(F), 1(T)),最后已知traffic flow作为decoder 的初始输入
    test_decoder_input_start0 = np.squeeze(test_decoder_input_start0, 2)  # (B,N,T(1))
    test_decoder_input0 = np.concatenate((test_decoder_input_start0, test_target_norm0[:, :, :-1]), axis=2)  # (B, N, T)

    test_x_tensor0 = torch.from_numpy(test_x0).type(torch.FloatTensor).to(DEVICE)  # (B, N, F, T)
    test_decoder_input_tensor0 = torch.from_numpy(test_decoder_input0).type(torch.FloatTensor).to(DEVICE)  # (B, N, T)
    test_target_tensor0 = torch.from_numpy(test_target_norm0).type(torch.FloatTensor).to(DEVICE)  # (B, N, T)

    test_dataset0 = torch.utils.data.TensorDataset(test_x_tensor0, test_decoder_input_tensor0, test_target_tensor0)

    test_loader0 = torch.utils.data.DataLoader(test_dataset0, batch_size=batch_size)

    ##dataset1
    test_decoder_input_start1 = test_x1[:, :, 0:1, -1:]  # (B, N, 1(F), 1(T)),最后已知traffic flow作为decoder 的初始输入
    test_decoder_input_start1 = np.squeeze(test_decoder_input_start1, 2)  # (B,N,T(1))
    test_decoder_input1 = np.concatenate((test_decoder_input_start1, test_target_norm1[:, :, :-1]), axis=2)  # (B, N, T)

    test_x_tensor1 = torch.from_numpy(test_x1).type(torch.FloatTensor).to(DEVICE)  # (B, N, F, T)
    test_decoder_input_tensor1 = torch.from_numpy(test_decoder_input1).type(torch.FloatTensor).to(DEVICE)  # (B, N, T)
    test_target_tensor1 = torch.from_numpy(test_target_norm1).type(torch.FloatTensor).to(DEVICE)  # (B, N, T)

    test_dataset1 = torch.utils.data.TensorDataset(test_x_tensor1, test_decoder_input_tensor1, test_target_tensor1)

    test_loader1 = torch.utils.data.DataLoader(test_dataset1, batch_size=batch_size)

    ##dataset2
    test_decoder_input_start2 = test_x2[:, :, 0:1, -1:]  # (B, N, 1(F), 1(T)),最后已知traffic flow作为decoder 的初始输入
    test_decoder_input_start2 = np.squeeze(test_decoder_input_start2, 2)  # (B,N,T(1))
    test_decoder_input2 = np.concatenate((test_decoder_input_start2, test_target_norm2[:, :, :-1]), axis=2)  # (B, N, T)

    test_x_tensor2 = torch.from_numpy(test_x2).type(torch.FloatTensor).to(DEVICE)  # (B, N, F, T)
    test_decoder_input_tensor2 = torch.from_numpy(test_decoder_input2).type(torch.FloatTensor).to(DEVICE)  # (B, N, T)
    test_target_tensor2 = torch.from_numpy(test_target_norm2).type(torch.FloatTensor).to(DEVICE)  # (B, N, T)

    test_dataset2 = torch.utils.data.TensorDataset(test_x_tensor2, test_decoder_input_tensor2, test_target_tensor2)

    test_loader2 = torch.utils.data.DataLoader(test_dataset2, batch_size=batch_size)
    ##dataset3
    test_decoder_input_start3 = test_x3[:, :, 0:1, -1:]  # (B, N, 1(F), 1(T)),最后已知traffic flow作为decoder 的初始输入
    test_decoder_input_start3 = np.squeeze(test_decoder_input_start3, 2)  # (B,N,T(1))
    test_decoder_input3 = np.concatenate((test_decoder_input_start3, test_target_norm3[:, :, :-1]), axis=2)  # (B, N, T)

    test_x_tensor3 = torch.from_numpy(test_x3).type(torch.FloatTensor).to(DEVICE)  # (B, N, F, T)
    test_decoder_input_tensor3 = torch.from_numpy(test_decoder_input3).type(torch.FloatTensor).to(DEVICE)  # (B, N, T)
    test_target_tensor3 = torch.from_numpy(test_target_norm3).type(torch.FloatTensor).to(DEVICE)  # (B, N, T)

    test_dataset3 = torch.utils.data.TensorDataset(test_x_tensor3, test_decoder_input_tensor3, test_target_tensor3)
    ...
    print('test1:', test_x_tensor1.size(), test_decoder_input_tensor1.size(), test_target_tensor1.size())
    print('test2:', test_x_tensor2.size(), test_decoder_input_tensor2.size(), test_target_tensor2.size())
    print('test3:', test_x_tensor3.size(), test_decoder_input_tensor3.size(), test_target_tensor3.size())
    return train_loader, train_target_tensor, val_loader, val_target_tensor, test_loader0, test_target_tensor0,test_loader1, test_target_tensor1,test_loader2, test_target_tensor2,test_loader3, test_target_tensor3, _max, _min,_max1,_min1,_max2,_min2,_max3,_min3

def load_graphdata_normY_channel1_match(graph_signal_matrix_filename,graph_signal_matrix_filename_test1,graph_signal_matrix_filename_test2,graph_signal_matrix_filename_test3, num_of_hours, num_of_days, num_of_weeks, DEVICE, batch_size,match1,match2, match3, shuffle=True, percent=1.0):
    '''
    将x,y都处理成归一化到[-1,1]之前的数据;
    每个样本同时包含所有监测点的数据，所以本函数构造的数据输入时空序列预测模型；
    该函数会把hour, day, week的时间串起来；
    ...
    # dataset 1
    test_x1 = file_data_test1['test_x']
    test_x1 = test_x1[:,list(match1.values()),:]
    test_x1 = test_x1[:, :, 0:1, :]
    test_target1 = file_data_test1['test_target']
    test_target1 = test_target1[:, list(match1.values()), :]
    test_timestamp1 = file_data_test1['test_timestamp']
    #dataset 2
    test_x2 = file_data_test2['test_x']
    test_x2 = test_x2[:,list(match2.values()),:]
    test_x2 = test_x2[:, :, 0:1, :]
    test_target2 = file_data_test2['test_target']
    test_target2 = test_target2[:,list(match2.values()),:]
    test_timestamp2 = file_data_test2['test_timestamp']
    #dataset 3
    test_x3 = file_data_test3['test_x']
    test_x3 = test_x3[:, list(match3.values()), :]
    test_x3 = test_x3[:, :, 0:1, :]
    test_target3 = file_data_test3['test_target']
    test_target3 = test_target3[:, list(match3.values()), :]
    test_timestamp3 = file_data_test3['test_timestamp']
...
def load_graphdata_normY_channel1_transfer(graph_signal_matrix_filename, graph_signal_matrix_filename_test1,
                                  graph_signal_matrix_filename_test2, graph_signal_matrix_filename_test3, num_of_hours,
                                  num_of_days, num_of_weeks, DEVICE, batch_size, shuffle=True, percent=1.0):
    '''
    将x,y都处理成归一化到[-1,1]之前的数据;
    每个样本同时包含所有监测点的数据，所以本函数构造的数据输入时空序列预测模型；
    该函数会把hour, day, week的时间串起来；
    ...
    filename_test1 = os.path.join(dirpath_test1,
                                  file_test1 + '_r' + str(num_of_hours) + '_d' + str(num_of_days) + '_w' + str(
                                      num_of_weeks) + '.npz')
    file_test2 = os.path.basename(graph_signal_matrix_filename_test2).split('.')[0]
    ...
    val_timestamp = file_data['val_timestamp']
    ##dataset 1
    val_x1 = file_data_test1['val_x']
    val_x1 = val_x1[:, :, 0:1, :]
    val_target1 = file_data_test1['val_target']
    val_timestamp1 = file_data_test1['val_timestamp']
    # ##dataset 2
    # val_x2 = file_data_test2['val_x']
    # val_x2 = val_x2[:, :, 0:1, :]
    # val_target1 = file_data_test2['val_target']
    # val_timestamp2 = file_data_test2['val_timestamp']
    # ##dataset 3
    # val_x3 = file_data_test3['val_x']
    # val_x3 = val_x3[:, :, 0:1, :]
    # val_target3 = file_data_test3['val_target']
    # val_timestamp3 = file_data_test3['val_timestamp']
    ...
    train_target_norm1 = max_min_normalization(train_target1, _max1[:, :, 0, :], _min1[:, :, 0, :])
    val_target_norm1 = max_min_normalization(val_target1, _max1[:, :, 0, :], _min1[:, :, 0, :])
    test_target_norm1 = max_min_normalization(test_target1, _max1[:, :, 0, :], _min1[:, :, 0, :])

    # test_target_norm1 = max_min_normalization(test_target1, _max[:, :, 0, :], _min[:, :, 0, :])

    test_target_norm2 = max_min_normalization(test_target2, _max1[:, :, 0, :], _min1[:, :, 0, :])

    test_target_norm3 = max_min_normalization(test_target3, _max1[:, :, 0, :], _min1[:, :, 0, :])
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=shuffle)
    # datset 1
    train_decoder_input_start1 = train_x1[:, :, 0:1, -1:]  # (B, N, 1(F), 1(T)),最后已知traffic flow作为decoder 的初始输入
    train_decoder_input_start1 = np.squeeze(train_decoder_input_start1, 2)  # (B,N,T(1))
    train_decoder_input1 = np.concatenate((train_decoder_input_start1, train_target_norm1[:, :, :-1]), axis=2)  # (B, N, T)

    train_x_tensor1 = torch.from_numpy(train_x1).type(torch.FloatTensor).to(DEVICE)  # (B, N, F, T)
    train_decoder_input_tensor1 = torch.from_numpy(train_decoder_input1).type(torch.FloatTensor).to(DEVICE)  # (B, N, T)
    train_target_tensor1 = torch.from_numpy(train_target_norm1).type(torch.FloatTensor).to(DEVICE)  # (B, N, T)

    train_dataset1 = torch.utils.data.TensorDataset(train_x_tensor1, train_decoder_input_tensor1, train_target_tensor1)

    train_loader1 = torch.utils.data.DataLoader(train_dataset1, batch_size=batch_size, shuffle=shuffle)
    # dataset 2
    ...
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=batch_size)
    # dataset 1
    val_decoder_input_start1 = val_x1[:, :, 0:1, -1:]  # (B, N, 1(F), 1(T)),最后已知traffic flow作为decoder 的初始输入
    val_decoder_input_start1 = np.squeeze(val_decoder_input_start1, 2)  # (B,N,T(1))
    val_decoder_input1 = np.concatenate((val_decoder_input_start1, val_target_norm1[:, :, :-1]), axis=2)  # (B, N, T)

    val_x_tensor1 = torch.from_numpy(val_x1).type(torch.FloatTensor).to(DEVICE)  # (B, N, F, T)
    val_decoder_input_tensor1 = torch.from_numpy(val_decoder_input1).type(torch.FloatTensor).to(DEVICE)  # (B, N, T)
    val_target_tensor1 = torch.from_numpy(val_target_norm1).type(torch.FloatTensor).to(DEVICE)  # (B, N, T)

    val_dataset1 = torch.utils.data.TensorDataset(val_x_tensor1, val_decoder_input_tensor1, val_target_tensor1)
    ...
    print('test2:', test_x_tensor2.size(), test_decoder_input_tensor2.size(), test_target_tensor2.size())
    print('test3:', test_x_tensor3.size(), test_decoder_input_tensor3.size(), test_target_tensor3.size())
    return train_loader, train_target_tensor, val_loader, val_target_tensor, train_loader1, train_target_tensor1, val_loader1, val_target_tensor1, test_loader0, test_target_tensor0, test_loader1, test_target_tensor1, test_loader2, test_target_tensor2, test_loader3, test_target_tensor3, _max, _min,_max1,_min1


def load_graphdata_normY_channel1_test(graph_signal_matrix_filename, num_of_hours,
                                  num_of_days, num_of_weeks, DEVICE, batch_size, shuffle=True, percent=1.0):
    '''
    将x,y都处理成归一化到[-1,1]之前的数据;
    每个样本同时包含所有监测点的数据，所以本函数构造的数据输入时空序列预测模型；
    ...
    filename = os.path.join(dirpath,
                            file + '_r' + str(num_of_hours) + '_d' + str(num_of_days) + '_w' + str(
                                num_of_weeks) + '.npz')
    file_test = os.path.basename(graph_signal_matrix_filename).split('.')[0]
    dirpath_test = os.path.dirname(graph_signal_matrix_filename)

    filename_test = os.path.join(dirpath_test,
                                  file_test + '_r' + str(num_of_hours) + '_d' + str(num_of_days) + '_w' + str(
                                      num_of_weeks) + '.npz')

    print('load file:', filename)

    file_data = np.load(filename)
    file_data_test = np.load(filename_test)
    ...
    test_x = file_data_test['test_x']
    test_x = test_x[:, :, 0:1, :]
    test_target = file_data_test['test_target']
    test_timestamp = file_data_test['test_timestamp']
